...
# 2. Definição da pasta de logs
PASTA_LOGS = DIRETORIO_BASE / "logs"
PASTA_LOGS.mkdir(parents=True, exist_ok=True)

# 3. ADICIONE/CORRIJA ESTA LINHA:
...
# ============================================================
# FUNCAO: exportar_csv_com_selecao_modulo() - MELHORADA
# ============================================================


def exportar_csv_com_selecao_modulo():
    """
    Exporta CSV com selecao de modulo e escolha do separador
    ...
    # ============================================================
    # PERGUNTAR QUAL SEPARADOR USAR
    # ============================================================
    print("\n" + "=" * 60)
    print("  SELECIONE O SEPARADOR DO CSV")
    print("=" * 60)
    print("")
    print("   [1] - Ponto e virgula (;) - Recomendado para Excel PT-BR/PT-PT")
    print("   [2] - Virgula (,) - Recomendado para Excel EN-US/Internacional")
    print("   [3] - Tab (\\t) - Para sistemas que usam tabulacao")
    print("   [4] - Ambos (gerar dois arquivos: ; e ,)")
    print("")
    print("   [Enter] - Usar padrao (ponto e virgula)")

    separador_opcao = input("\nEscolha (1-4 ou Enter): ").strip()
    ...
    # Se Enter, usa ponto e virgula (padrao)
    if separador_opcao == "":
        separador = ";"
        return gerar_csv(modulo_selecionado, separador)
    elif separador_opcao in separadores:
        separador = separadores[separador_opcao]
        return gerar_csv(modulo_selecionado, separador)
    elif separador_opcao == "4":
        # Gerar ambos
        print("\nGerando ambos os formatos...")
        print("   CSV com ponto e virgula (;)")
        arquivo1 = gerar_csv(modulo_selecionado, ";")
        print("   CSV com virgula (,)")
        arquivo2 = gerar_csv(modulo_selecionado, ",")

        if arquivo1 and arquivo2:
            print("\nAMBOS os CSVs gerados com sucesso!")
            print(f"   Ponto e virgula: {Path(arquivo1).name}")
            print(f"   Virgula: {Path(arquivo2).name}")
        return None
    else:
        print("[AVISO] Opcao invalida! Usando ponto e virgula como padrao.")
        return gerar_csv(modulo_selecionado, ";")


# ============================================================
# FUNCAO: gerar_csv() - MELHORADA COM PARAMETRO SEPARADOR
# ============================================================


def gerar_csv(modulo_selecionado=None, separador=";"):
    """
    Gera o CSV com as colunas: ID, data_hora, status, modulo, observacao, utilizador

    Args:
        modulo_selecionado: Nome do modulo para filtrar (ou None para todos)
        separador: Caractere separador (; , ou \t)
    """
    global estado_sistema
    ...
    # Nome do separador para exibicao
    nome_separador = {";": "ponto_virgula", ",": "virgula", "\t": "tab"}.get(
        separador, "ponto_virgula"
    )

    # Simbolo do separador para exibicao
    simbolo_separador = {
        ";": "ponto e virgula (;)",
        ",": "virgula (,)",
        "\t": "tab (\\t)",
    }.get(separador, "ponto e virgula (;)")

    print(f"\n   A gerar CSV com separador '{simbolo_separador}'...")
    ...
    # Nome do arquivo com indicacao do separador
    if modulo_selecionado:
        nome_arquivo = f"csv_{modulo_selecionado}_{nome_separador}_{timestamp}.csv"
    else:
        nome_arquivo = f"csv_todos_eventos_{nome_separador}_{timestamp}.csv"
    ...
    try:
        ...
        print(f"   Separador: {simbolo_separador}")
...
def menu():
    ...
    while True:
        try:
            ...
            print(" 5. Exportar CSV (com selecao de modulo e separador)")
